...
index.html                 ← 新增第 4 个 Tab「CLIP V2 上传剪辑」+ 上传表单 + 进度 + 下载
...
V2 作为独立的第 4 个 Tab，与 V1 CLIP Tab 互不干扰。

```
┌─ Tab 导航 ───────────────────────────────────────────────────┐
│ RADAR 素材搜索 │ COPY 文案生成 │ CLIP 智能剪辑 │ CLIP V2 上传剪辑 │
└─────────────────────────────────────────────────────────────┘
                                │                   │
                                ▼ (V1)              ▼ (V2)
                          YouTube 链接         📁 视频文件上传
                          字幕文本输入           剪辑描述 textarea
                          定位时间段             目标时长 15/20/25/30s
                          JSON 输出             下载剪好的 .mp4
```
...
└─────────────────────────────────────────────────┘
  ...
  TARGET    [15s]  [20s]  [25s]  [30s]

  [AI 視覺模型分析畫面 + 智能剪輯]     [分析 →]

┌─ 結果區 ────────────────────────────────────────┐
│  剪輯完成  clip_xxx.mp4 (18.5s)                 │
│  42.0s ~ 60.5s                                  │
│  候選人回答時表情生動，內容切題                     │
│  [⬇ 下載影片]                                   │
└─────────────────────────────────────────────────┘
...
**用户操作流程：**
1. 拖拽或点击上传本地视频文件
2. 输入剪辑描述（自然语言，如「找主持人追問來賓那段」）
3. 选择目标时长（15/20/25/30s，默认 20s）
4. 点击「分析」
5. 等待处理（显示 loader 动画）
6. 结果区显示剪好的视频信息 + 下载按钮
